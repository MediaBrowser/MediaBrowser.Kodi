#################################################################################################
# Start of ThemeMovies Thread
# plays theme movie where applicable
#################################################################################################
...
class ThemeMoviesThread(threading.Thread):
    ...
    def __init__(self, *args):
        ...
        xbmc.log("XBMB3C ThemeMovieThread -> Log Level:" +  str(self.logLevel))
        
        threading.Thread.__init__(self, *args)    
    
    def logMsg(self, msg, level = 1):
        if(self.logLevel >= level):
            xbmc.log("XBMB3C ThemeMovieThread -> " + msg)
    
    def run(self):
        self.logMsg("Started")
        self.updateThemeMovie()
    ...
    def updateThemeMovie(self):
        self.logMsg("updateThemeMovie Called")
        ...
        if id != newid:
            return
        self.logMsg("updateThemeMovie itemGUID : " + id)
        if self.isPlayingZone() and self.isChangeTheme():
            self.themeId = id 
            themeUrl = "http://" + mb3Host + ":" + mb3Port + "/mediabrowser/Items/" + id + "/ThemeVideos?format=json"
            self.logMsg("updateThemeMovie themeUrl : " + themeUrl)
            if themeUrl not in self.themeMap:
                try:
                    ...
                    self.logMsg("updateThemeMovie urlopen : " + str(e) + " (" + themeUrl + ")", level=0)
                    return
                theme = json.loads(jsonData)     
               
                if(theme == None):
                    theme = []
                self.logMsg("updateThemeMovie added theme to map : " + themeUrl)    
                self.themeMap[themeUrl] = theme
            elif themeUrl in self.themeMap:
                theme = self.themeMap.get(themeUrl)
                self.logMsg("updateThemeMovie retrieved theme from map : " + themeUrl)
            
            themeItems = theme.get("Items")
            if themeItems != []:
                themePlayUrl = PlayUtils().getPlayUrl(mb3Host + ":" + mb3Port,themeItems[0].get("Id"),themeItems[0])
                self.logMsg("updateThemeMovie ThemeMoviePath : " + str(themePlayUrl))
    ...
    def stop(self, forceStop = False):
        # Only stop if playing video
        if xbmc.Player().isPlayingVideo() or forceStop == True:
    ...
    def isChangeTheme(self):
        ...
        if id != "":
            ...
            addonSettings = xbmcaddon.Addon(id='plugin.video.xbmb3c') 
            if addonSettings.getSetting('useThemeMovie') == "true":
              # cool theme music is on continue
              if id == self.themeId:
                  # same as before now do we need to restart 
                  if addonSettings.getSetting('loopThemeMovie') == "true" and xbmc.Player().isPlayingVideo() == False:
                      return True
              if id != self.themeId:
                  # new id return true
                  return True  
